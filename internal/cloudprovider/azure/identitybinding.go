...
func LoadIdentityBindingConfigFromEnv() (*IdentityBindingConfig, error) {
	...
	if apiServerHost == "" {
		// The AKS webhook injects the token endpoint as a full URL; preserve the
		// host and any explicit port so the dialer can target the exact authority.
		if proxy := strings.TrimSpace(os.Getenv(EnvAKSIdentityBindingTokenProxy)); proxy != "" {
			host, err := authorityFromURL(proxy)
			if err != nil {
...
// authorityFromURL extracts the host[:port] authority from a URL and rejects
// path/query/fragment components so the dialer targets the exact injected
// token proxy authority.
func authorityFromURL(raw string) (string, error) {
	u, err := url.Parse(raw)
	if err != nil {
		return "", err
	}
	...
	if u.Path != "" && u.Path != "/" {
		return "", fmt.Errorf("path %q is not allowed in URL %q", u.Path, raw)
	}
	if u.RawQuery != "" {
		return "", fmt.Errorf("query is not allowed in URL %q", raw)
	}
	if u.Fragment != "" {
		return "", fmt.Errorf("fragment is not allowed in URL %q", raw)
	}
	return u.Host, nil
}
...
func newIdentityBindingTransport(sniName, apiServerHost string, caPool *x509.CertPool) *http.Transport {
	...
	transport.DialContext = func(ctx context.Context, network, addr string) (net.Conn, error) {
		...
		// Route to the API server host (IP/FQDN, optionally with an explicit
		// port from the injected token proxy URL) regardless of the SNI host in
		// the request URL. A FQDN is resolved by the dialer at connect time.
		fixedAddr := net.JoinHostPort(apiServerHost, port)
		if _, _, err := net.SplitHostPort(apiServerHost); err == nil {
			fixedAddr = apiServerHost
		}
		dialer := &net.Dialer{
