...
func TestLoadIdentityBindingConfigFromEnv(t *testing.T) {
	tests := []struct {
		...
		{
			name: "AKS token proxy preserves explicit port",
			env: map[string]string{
				EnvAKSIdentityBindingSNIName:    "sni.ests.aks",
				EnvAKSIdentityBindingTokenProxy: "https://apiserver.hcp.westus2.azmk8s.io:8443",
			},
			assert: func(t *testing.T, cfg *IdentityBindingConfig) {
				if cfg.APIServerHost != "apiserver.hcp.westus2.azmk8s.io:8443" {
					t.Errorf("unexpected APIServerHost: %s", cfg.APIServerHost)
				}
			},
		},
		{
			name: "AKS token proxy with path errors",
			env: map[string]string{
				EnvAKSIdentityBindingSNIName:    "sni.ests.aks",
				EnvAKSIdentityBindingTokenProxy: "https://apiserver.hcp.westus2.azmk8s.io/token",
			},
			expectError: true,
		},
	}
...
func TestAuthorityFromURL(t *testing.T) {
	t.Run("valid url with scheme and port", func(t *testing.T) {
		host, err := authorityFromURL("https://apiserver.example.com:443")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
		if host != "apiserver.example.com:443" {
			t.Errorf("expected host apiserver.example.com:443, got %s", host)
		}
	})

	t.Run("valid url without port", func(t *testing.T) {
		host, err := authorityFromURL("https://apiserver.example.com")
		if err != nil {
			t.Fatalf("unexpected error: %v", err)
		}
	...
	t.Run("path rejected", func(t *testing.T) {
		if _, err := authorityFromURL("https://apiserver.example.com/token"); err == nil {
			t.Fatal("expected error for URL with path")
		}
	})

	t.Run("query rejected", func(t *testing.T) {
		if _, err := authorityFromURL("https://apiserver.example.com?x=y"); err == nil {
			t.Fatal("expected error for URL with query")
		}
	})

	t.Run("no host", func(t *testing.T) {
		if _, err := authorityFromURL("not-a-url"); err == nil {
			t.Fatal("expected error for URL without host")
		}
	})

	t.Run("unparsable url", func(t *testing.T) {
		if _, err := authorityFromURL("http://[::1"); err == nil {
