...
func TestConvertJWKAlgToHash(t *testing.T) {
	tests := []struct {
		name        string
		algorithm   string
		...
		{
			name:      "PS384 algorithm",
			algorithm: "PS384",
			expected:  crypto.SHA384,
		},
		{
			name:      "ES384 algorithm",
			algorithm: "ES384",
			expected:  crypto.SHA384,
		},
		{
		...
		{
			name:      "PS512 algorithm",
			algorithm: "PS512",
			expected:  crypto.SHA512,
		},
		{
			name:      "ES512 algorithm",
			algorithm: "ES512",
			expected:  crypto.SHA512,
		},
		{
			name:      "EdDSA algorithm (no hash)",
			algorithm: "EdDSA",
		...
		},
		{
			...
			algorithm:   "",
			expectError: true,
			errorMsg:    "empty algorithm",
		},
		{
			name:        "unsupported algorithm",
			algorithm:   "HS256",
			expectError: true,
			errorMsg:    "unsupported algorithm: \"HS256\"",
		},
		{
			name:        "unknown algorithm",
			algorithm:   "UNKNOWN_ALG",
			expectError: true,
			errorMsg:    "unsupported algorithm: \"UNKNOWN_ALG\"",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			hash, err := convertJWKAlgToHash(tt.algorithm)
			if tt.expectError {
				assert.Error(t, err)
				assert.Contains(t, err.Error(), tt.errorMsg)
			} else {
