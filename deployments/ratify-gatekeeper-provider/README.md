...
| `terminationGracePeriodSeconds`           | Time (seconds) given to drain in-flight requests on shutdown or scale-down.                                                                                                                          | `30`                                            |
| `preStopSleepSeconds`                     | preStop sleep (native lifecycle action, Kubernetes >= 1.30) to let Service endpoints deregister before the pod stops serving. Set to `0` to disable.                                                  | `5`                                             |
| `podDisruptionBudget.enabled`             | Create a PodDisruptionBudget for the provider. Enable when running more than one replica.                                                                                                            | `false`                                         |
| `podDisruptionBudget.minAvailable`        | Minimum number of available pods for the PodDisruptionBudget.                                                                                                                                        | `1`                                             |
...
| `cosign.ignoreCTLog`                    | Whether to ignore certificate transparency log verification in Cosign verifier. Only applies to keyless verification; key-based verification always ignores the certificate transparency log.                  | `false`                                         |
