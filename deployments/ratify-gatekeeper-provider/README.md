...
| `provider.leaderElection.enabled`         | Enable leader election so that, when running multiple replicas, only one replica writes Executor CRD status. All replicas still serve verification and reconcile their own executor configuration. | `false`                                         |
