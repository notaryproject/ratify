...
| `autoscaling.enabled`                     | Enable a HorizontalPodAutoscaler for the provider Deployment. Requires metrics-server and resource requests; when enabled, `replicaCount` is ignored.                                                 | `false`                                         |
| `autoscaling.minReplicas`                 | Minimum number of replicas when autoscaling is enabled.                                                                                                                                              | `1`                                             |
| `autoscaling.maxReplicas`                 | Maximum number of replicas when autoscaling is enabled.                                                                                                                                              | `5`                                             |
| `autoscaling.targetCPUUtilizationPercentage` | Target average CPU utilization (percentage of the CPU request) that triggers scaling.                                                                                                            | `80`                                            |
